...
class FBP(nn.Module):
        ...
        if self.args.geometry == 'fan':
            if self.args.mode == 'equally_spaced':
                ...
                self.s_range = self.args.SCD/self.args.SDD*(self.args.det_interval*
                        torch.linspace((1-self.args.num_det)/2, (self.args.num_det-1)/2, self.args.num_det, dtype=self.args.datatype)+self.shift)
                ...
                self.s_range = 1/self.args.SDD*(self.args.det_interval*
                        torch.linspace((1-self.args.num_det)/2, (self.args.num_det-1)/2, self.args.num_det, dtype=self.args.datatype)+self.shift)
                gamma = (torch.arange(1,args.num_det+1, dtype=self.args.datatype) - y_center) * self.delta_s #gamma = s_range in this geometry
    ...
    def param_setting(self):
        x = self.args.recon_interval*torch.linspace((1-self.args.recon_size[0])/2, (self.args.recon_size[0]-1)/2, 
                                                    self.args.recon_size[0], dtype=self.args.datatype) + self.args.ROIx
        y = self.args.recon_interval*torch.linspace((1-self.args.recon_size[1])/2, (self.args.recon_size[1]-1)/2, 
                                                    self.args.recon_size[1], dtype=self.args.datatype) + self.args.ROIy
        ...
        if self.args.geometry == 'fan':
            if self.args.mode == 'equally_spaced':
                U = (self.args.SCD + r[None, :, :] * torch.sin(self.beta[:, None, None] -
                        phi[None, :, :])) / self.args.SCD  # (view,x,y)
                s_xy = - r[None, :, :] * torch.cos(self.beta[:, None, None] - phi[None, :, :]) / U  #s value of each coord (view,x,y)
                s_xy_n = torch.reshape(s_xy / self.s_range[-1], [self.args.view, -1])  # convert range as [-1 1], (view,x*y)
                view_xy_n = torch.tile(torch.linspace(-1, 1, int(self.args.view / self.args.num_split), dtype=self.args.datatype)[:, None],
                ...
                L = torch.sqrt(torch.pow(self.args.SCD + r[None, :, :] * torch.sin(self.beta[:, None, None] - phi[None, :, :]), 2) 
                                    + torch.pow(r[None, :, :] * torch.cos(self.beta[:, None, None] - phi[None, :, :]), 2))
                s_xy = torch.atan2(- r[None, :, :] * torch.cos(self.beta[:, None, None]- phi[None, :, :]), 
                                  self.args.SCD + r * torch.sin(self.beta[:, None, None] - phi[None, :, :])) #s value of each coord (view,x,y)
                s_xy_n = torch.reshape(s_xy / self.s_range[-1], [self.args.view, -1])  # convert range as [-1 1], (view,x*y)
                view_xy_n = torch.tile(torch.linspace(-1, 1, int(self.args.view / self.args.num_split), dtype=self.args.datatype)[:, None],
                                       [self.args.num_split, self.args.recon_size[0] * self.args.recon_size[1]])
                grid = torch.stack([s_xy_n, view_xy_n], dim=2)  # (view,x*y,2)
    ...
    def forward(self, sinogram):
        batch, ch, N, view = sinogram.shape
        recon_img = self.prepare(torch.zeros([batch, ch, self.args.recon_size[0], self.args.recon_size[1]], dtype=self.args.datatype))
        sinogram = torch.fft.fft(sinogram * self.cos_weight[None, None, :, None], n=self.args.Npow2, dim=-2) \
                                          * self.recon_filter[None, :, :, None] * self.window[None, :, :, None]
        sinogram = torch.real(self.delta_s * torch.fft.ifft(sinogram, dim = -2))
        sinogram = sinogram[:, :, N-1:2*N-1, :]  # cut zero-padded components
        
        view_arr = np.arange(self.args.view)
        split_view = np.split(view_arr, self.args.num_split, axis=0)
        view_batch = int(self.args.view / self.args.num_split)

        for i in split_view:
            recon_temp = torch.nn.functional.grid_sample(sinogram[:,:,:,i].permute(0,1,3,2),
                                            torch.tile(self.grid[None, i, :, :], [batch, 1, 1, 1]),
                                            mode='nearest', padding_mode='zeros', align_corners=False) # (batch, ch, view, x*y)
            recon_temp = torch.sum(recon_temp / self.square[None, None, i, :], dim=-2) #(batch, ch, x*y)
            recon_img += self.d_beta * recon_temp[:, :, :, None] \
        ...
        return torch.rot90(recon_img.permute(0,1,3,2), k = 2, dims = [2,3])
    ...
    def gen_mask(self):
        ...
        x = torch.linspace(-(self.args.recon_size[0] - 1) / 2, (self.args.recon_size[0] - 1) / 2, self.args.recon_size[0])
        y = torch.linspace(-(self.args.recon_size[1] - 1) / 2, (self.args.recon_size[1] - 1) / 2, self.args.recon_size[1])
        x_mat, y_mat = torch.meshgrid(x, y, indexing='xy')
